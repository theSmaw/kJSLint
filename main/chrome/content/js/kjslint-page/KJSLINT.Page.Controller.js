...
window.extensions.KJSLINT.Page.Controller = (function () {
        ...
        if (!prefsObject[currentPath]) {
            ...
            for (property in tempObj) {
                if (options.hasOwnProperty(property)) {
                    prefsObject[currentPath][property] = options[property];
                }
            }
        }
    }
    ...
    function eventTabChanged(e) {
        ...
        try {
            currentPath = e.originalTarget.document.file.URI.replace(/\"/g, '');
        }
        catch (err) {
            currentPath = 'temp';
        }
    ...
    function eventUnload() {
        window.extensions.KJSLINT.Options.Preferences.save(prefsObject);
    ...
    function viewShow(myData) {
        ...
        if (!myData.errors) {
            ...
            window.extensions.KJSLINT.KomodoAdaptor.ensureTabShown(constErrorsTabId);
            ...
            window.extensions.KJSLINT.KomodoAdaptor.ensureTabShown(constFunctionsTabId);
        }
        else {
    ...
    function run() {
        ...
        data = window.extensions.KJSLINT.JSLINTAdaptor(ko.views.manager.currentView.document.buffer, prefsObject[currentPath]);
        viewShow(data);
    ...
    function viewShowOptions() {
        window.extensions.KJSLINT.KomodoAdaptor.ensureTabShown(constOptionsTabId);
        if (!elOptionsPanel) {
            elOptionsPanel = document.getElementById(constOptionsTabPanelId);
        }
